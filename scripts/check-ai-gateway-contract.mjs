const apiBaseUrl = normalizeBaseUrl(
  process.env.AI_GATEWAY_BASE_URL
    || process.env.VITE_AI_GATEWAY_BASE_URL
    || process.env.SUB2API_BASE_URL
    || process.env.VITE_SUB2API_BASE_URL,
  '/api/v1'
);

const email = process.env.AI_GATEWAY_EMAIL || process.env.SUB2API_EMAIL;
const password = process.env.AI_GATEWAY_PASSWORD || process.env.SUB2API_PASSWORD;

if (!apiBaseUrl || !email || !password) {
  console.error([
    'Missing required environment variables.',
    'Required for the provider-neutral check:',
    '  AI_GATEWAY_BASE_URL=https://gateway.example.com',
    '  AI_GATEWAY_EMAIL=<your account email>',
    '  AI_GATEWAY_PASSWORD=<your account password>',
    '',
...
try {
  ...
  if (login.requires_2fa) {
    ...
    console.log('AI gateway contract is reachable, but this account requires 2FA.');
  ...
  console.table(checks.map(([name, ok]) => ({ check: name, ok })));
  console.log('AI gateway contract check passed.');
} catch (error) {
  console.table(checks.map(([name, ok]) => ({ check: name, ok })));
  console.error(`AI gateway contract check failed: ${error.message}`);
  process.exit(1);
}
...
async function request(path, options) {
  ...
  if (payload && typeof payload === 'object' && 'code' in payload) {
    ...
    throw new Error(payload.message || `GATEWAY_CODE_${payload.code}`);
  }
